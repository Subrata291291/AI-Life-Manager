        ...
        <div className="main-div">
          ...
          <div className="row mb-4">
            ...
            <div className="col-6 col-md-3">
              <div className="card">
                <div className="card-body text-center">
                  <h6>In Progress</h6>
                  <h3>{tasks.filter(t => getTaskStatus(t) === "In Progress").length}</h3>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="card mb-4">
            <div className="card-body">
              <h5 className="mb-3">{editingId ? "Edit Task" : "Create New Task"}</h5>
                      ...
                      <tr key={task.id}>
                        ...
                        <td>
                          {(() => {
                            ...
                            const diffMs = new Date(task.end_time).getTime() - new Date(task.start_time).getTime();
                        ...
                        <td>
                          {getTaskStatus(task) === "Completed" && <span className="badge bg-success">Completed</span>}
                          {getTaskStatus(task) === "Upcoming" && <span className="badge bg-primary">Upcoming</span>}
                          {getTaskStatus(task) === "In Progress" && <span className="badge bg-info">In Progress</span>}
                          {getTaskStatus(task) === "Overdue" && <span className="badge bg-danger">Expired</span>}
                        </td>
                        <td>
                          <button className="btn btn-warning btn-sm me-2" onClick={() => {
                            setEditingId(task.id);
                            setFormData({ title: task.title, description: task.description, priority: task.priority, start_time: task.start_time, end_time: task.end_time });
                          }}>Edit</button>
                          {task.status !== "completed" && (
                            <button className="btn btn-success btn-sm me-2" onClick={() => updateStatus(task.id, "completed")}>Complete</button>
                          )}
